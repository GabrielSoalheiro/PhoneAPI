...
public class UpdateContactCommandValidator : AbstractValidator<UpdateContactCommand>
{
    ...
    public UpdateContactCommandValidator(IContactRepository contactRepository)
    {
        ...
        RuleFor(x => x.Telefone)
            ...
            .MustAsync(async (command, telefone, cancellation) =>
                !await _contactRepository.ExistsByTelefoneAsync(telefone, command.Id))
            .WithMessage("Este telefone já está cadastrado por outro contato");

        RuleFor(x => x.Email)
            .NotEmpty().WithMessage("Email é obrigatório")
            .EmailAddress().WithMessage("Email deve ter um formato válido")
            .MustAsync(async (command, email, cancellation) =>
                !await _contactRepository.ExistsByEmailAsync(email, command.Id))
            .WithMessage("Este email já está cadastrado por outro contato");
